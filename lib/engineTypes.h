 ...
 */
#pragma once
#ifndef RESYNTH_ENGINE_TYPES_H_
#define RESYNTH_ENGINE_TYPES_H_
 ...
 */
static inline CompareResult to_sort_result(gboolean value)
{
...
static inline CompareResult to_invert_sort_result(CompareResult value)
{
	return ((value == 1) ? -1 : 1);  // equality not a case
}
...
/*
 * Represents the boundary of a set of points
 */
typedef struct 
{
...
Bounds get_bounds(PointVector points, guint size)
{
...
Coordinates get_center(PointVector points, guint size)
{
	Coordinates center;
...
inline void swap_vector_elements(PointVector vector, guint size, guint i, guint j)
{
	Coordinates temp;
